...
1. What a **latent variable** is, and why we call it a *cause*
2. ***Classical Test Theory***: $X = T + E$ and its assumptions
3. Why reliability can be defined as a **proportion of variance**
4. What *true score* actually means — and why it is **not** "the real amount of the construct"
5. Consequences of CTT: number of items, attenuation, and sample dependence
6. The family of CTT models: *parallel*, *tau-equivalent*, *congeneric*
7. A glimpse of ***Item Response Theory*** and what it offers


# Turning assumptions into a model {background-color="#14497F" .center}
...
As long as assumptions are only a list, nothing can be computed from them. Once they are written as equations, we can **derive their consequences**.
:::
...
* $X$ — the ***observed score***. What you actually get from a participant.
...
::: {.callout-note}
#### What goes into $E$
The participant's physical state, a passing mood, an ambiguous item, noise in the room, a mis-click. CTT does **not separate** these sources. They are all collected into a single error term.
:::
...
All three say that error is **random**, not systematic. A consistent bias — for example every participant answering more positively because of social desirability — does **not** enter $E$.
:::

## Splitting score variance
...
Someone who scores **30** is really located within a range, not at a point. At 95% confidence (1.96, [see this figure](https://www.researchgate.net/profile/Avijit-Hazra-2/publication/320742650/figure/tbl1/AS:669269071237158@1536577589946/Critical-z-values-used-in-the-calculation-of-confidence-intervals.png)):
...
## Connect this back to Sessions 1 and 2
...
* If your participants are very uniform on the attribute, their range of true scores is narrow. Inter-item correlations shrink, and the **computed reliability drops** — even though the instrument is unchanged.
...
* **Item difficulty** in CTT is the proportion of participants endorsing an item. That number changes when the sample changes.
...
4. **If every one of your items is "easy to endorse", what will happen** to your score distribution and to your scale's ability to tell participants apart?
...
6. **$\alpha$ assumes the tau-equivalent model**, which is rarely satisfied — so $\omega$, which assumes the congeneric model, is usually the better choice.
