...
* **Today**: those assumptions get tidied into a **formal model** — the one we will use for the rest of the course.
...
* A latent variable is usually a **characteristic of the person supplying the data**, not of the object being asked about.
...
* If we ask shoppers to rate a store, we are measuring **shoppers' perceptions**, not properties of the store.
...
The name of your construct must reflect **whose data you collected**. This will be checked when you write your operational definition in Session 9.
:::
...
* Note the direction of the arrow: from construct **to** item. Not the other way round.
...
We will see shortly what that implies.
:::
...
## The definition is narrower than you might think
...
With $\rho = 0.64$, the largest correlation you could possibly obtain is $0.80$ — however strong the real relationship is.
:::
...
## Why this distinction is practical, not just terminological
...
::: {.callout-tip}
#### The stance we take
We will use CTT all semester because it is useful and workable. But you now know exactly **what it stands on**.
:::
...
::: {.callout-tip}
The larger the share of noise in your scores, the smaller every correlation you report. This happens automatically, without any mistake on your part.
:::
...
::: {.callout-note}
This is exactly the problem Item Response Theory sets out to solve. We turn to it in the final section.
:::
...
![](libs/model-pengukuran.png){fig-align="center" width="78%"}

::: aside
**1** marks the reference indicator; **a** marks loadings constrained to be equal; **b** marks error variances constrained to be equal.
:::
...
* In physical measurement, **20 kilograms means the same thing** whatever is being weighed. The instrument's properties do not depend on the object.
...
::: {.callout-note}
IRT is better described as a **family of models** than as a single procedure.
:::
...
::: {.callout-note}
#### Notice
The first two are precisely assumption 4 from Session 2. IRT does **not remove** assumptions — it demands them more strictly, and gives more back in return.
:::
