...
* **Final project**: each group of five students will **build a complete psychological scale, together with its manual**.
  + Add your group members' names to [this spreadsheet](https://docs.google.com/spreadsheets/d/1fTRKB_1MbFcJhqbmfC_gg8h5YDJlRPmh/edit?usp=sharing&ouid=100126014993390956583&rtpof=true&sd=true).
  + Access to the spreadsheet will be locked after Session 3.

* **First half (Sessions 1–7)**: laying the conceptual groundwork. What measurement is, which assumptions we rely on (some implicit and easy to miss), what a latent variable is, how to choose a response format, and what it means for scores to be reliable and valid.
...
::: {.callout-tip}
#### The first half sets up the second
...
* None of them is a **statistical** problem; a fancier statistical test would not solve any of them.
...
::: {.callout-warning}
#### Why this matters
If you take that shortcut, every conclusion in the study rests on an assumption nobody ever checked. The problem is that the data will still **look completely fine**. There will still be means, still be *p*-values, still be a plot you can put in a paper. But does the resulting data actually match what it claims to measure?
:::
...
Note that this is a **normative** question ("deserve"), which is very different from a technical one like "what's the scale's reliability coefficient?" The answer is not inside the data — it has to be justified on its own terms.
...
## Modeling measurement error statistically
...
Many early contributors to psychometrics worked within a **[eugenics](https://www.genome.gov/about-genomics/fact-sheets/Eugenics-and-Scientific-Racism)** framework. We still use their statistical tools today, but we are **not obligated to carry forward** the political assumptions and consequences behind them (e.g., forced sterilization or restricted reproduction for groups deemed "inferior").
:::
...
This course does not ask you to reject psychometrics, nor to swallow it whole. What it asks is that you be able to say: **I know exactly which assumptions I am borrowing, and what follows if they are wrong.**
:::
...
* The data are rich on professional status over time. On social support? Nothing. Only a few items asking **whether the participant was married**.
...
::: {.callout-tip}
#### One question to ask before accepting a null result
...
* Is that distance the same **for every participant**? For every item? In every language?
...
The instruments were simply noisy, not because you miscalculated or because your sample was too small.
:::
...
Two studies can both report on "depression", both use scales that are equally reliable, and **still not be measuring the same thing**. Comparing them is far more fragile than it looks.
...
* Researchers often economise by using instruments that are **too brief**, hoping to reduce participant burden.

* But a questionnaire too short to be reliable is **a bad idea**, however convenient it feels.

* A **reliable** questionnaire completed by half your participants yields more information than an **unreliable** one completed by all of them.
...
5. **Not every set of items is a scale.** What decides is the direction of causality between construct and items.
...
* That participants **can** and **will** report their internal states accurately.
