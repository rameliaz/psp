...
1. Three things we have been taking for granted — and the formats that give them up
2. **Thurstone**: items calibrated in advance by a panel of judges
3. ***Situational Judgement Tests***: measuring judgement rather than self-description
4. ***Forced-choice***: when participants must choose, and scores become **ipsative**
5. Why ipsative data breaks almost everything we built in Session 3
...
* **Ask a panel of judges** to sort each statement into 11 piles, from most opposed to most in favour.
  - Judges are **not** asked their own opinion; they judge **where the statement sits** on the attribute continuum ($\theta$).

* **Take the median** judge rating for each statement.
  + That is its **scale value**.
...
* Each statement has **one number** marking its position on the continuum.
  - That is what reflects its calibration.

* Statement 4 has a **very wide** interquartile range: the judges could not agree where it sits.
  - Statements like this are discarded, however well written.

* The surviving statements must be **spread** from end to end of the attribute continuum ($\theta$), so they represent the whole spectrum of the construct.
...
## How participants answer, and how it is scored
...
* **Inside IRT.** IRT-based methods **share the same goal** as Thurstone scaling.
  + The difficulty parameter $b$ from Session 3 is essentially a Thurstone scale value, except that IRT estimates it from participant data rather than from a judge panel.

* **Inside your own project.** In Session 11 you will ask a panel of experts to rate the relevance of each item and compute a [*Content Validity Index*](slides/pertemuan11-en.html).
...
* Rooted in personnel selection testing since the 1940s, and revived after [Motowidlo et al. (1990)](https://psycnet.apa.org/record/1991-11281-001) described it as a *low-fidelity simulation*.
...
::: {.callout-warning appearance="simple"}
[Ployhart & Ehrhart (2003)](https://onlinelibrary.wiley.com/doi/abs/10.1111/1468-2389.00222) showed that **changing the response instruction changes the test's psychometric properties** — both its reliability and the construct it measures.
:::
...
* **Concrete context.** participants are not asked to generalise about themselves but to react to a specific situation.
  - This sidesteps part of the introspective-access problem from Session 2.

* **Face relevance**, which makes it easier to accept in a selection setting.

* **Reasonable prediction of performance.** [McDaniel et al. (2001)](https://psycnet.apa.org/record/2001-01869-017) show SJTs predict job performance adequately.
...
* Matching attractiveness is itself a calibration task.
  + Once again, Thurstone-style work.

:::

## Ipsativity: the price
...
* As a result, **the total across all scales is constant** for every participant.
  + If one scale score goes up, another **must** go down.
...
* [Brown & Maydeu-Olivares (2011)](https://journals.sagepub.com/doi/abs/10.1177/0013164410375112) showed that with **IRT** modelling, normative scores can be recovered from forced-choice responses.

* The approach is called [***Thurstonian IRT***](https://pmc.ncbi.nlm.nih.gov/articles/PMC6713979/), because it builds on Thurstone's model of paired comparisons.
...
## All three are not fully realistic for your project
...
6. **Ipsative data forces negative correlations between scales** and makes between-person comparison invalid.
  + But this can be corrected with the right modelling technique ([Thurstonian IRT](https://pmc.ncbi.nlm.nih.gov/articles/PMC6713979/)).
