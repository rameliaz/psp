...
1. What we are actually choosing when we choose a response format
2. **Likert**: where it came from, and the four decisions you have to make
3. **Semantic Differential**: when adjective pairs work better than statements
4. **Guttman**: when items really are ordered by logic — and why it rarely works
5. Comparing the three
6. The closing question: **which assumptions does a response format change?**
...
* A response format is **a rule that maps a participant's internal state onto a number**.
...
* **Four points or fewer** compresses variation between participants and lowers precision.
...
First, **whether participants can actually discriminate**. If they cannot tell seven levels apart, seven points only add noise.
...
**Forces** participants to take a side, however weakly.
...
* If every item has a midpoint, some participants will learn they can **always choose it** and finish faster.

* The midpoint becomes the safe, lowest-effort option — not a report of their state.

* [Krosnick (1991)](https://onlinelibrary.wiley.com/doi/abs/10.1002/acp.2350050305) calls this ***satisficing***: giving an answer that is good enough rather than the most accurate one.
...
When a midpoint is available it is often **the most frequently chosen option**. If that happens in your pilot data in Session 12, do not immediately conclude your participants are genuinely neutral.
:::
...
* The reason is simple: if only the ends are labelled, each participant decides for themselves what the points in between mean.
...
Consider this arrangement, adapted from DeVellis:

Very Helpful ➡️ Not Very Helpful ➡️ Somewhat Helpful ➡️ Not at All Helpful

**The problem**: "Somewhat" and "Not Very" are hard to tell apart, even with the order reversed.
:::
...
::: {.callout-tip}
#### Agreement anchors invite agreement
Agreement anchors invite participants simply to agree. [Weijters et al. (2010)](https://psycnet.apa.org/record/2010-04293-009) reinforce this point, showing that scale format itself shapes which response styles appear.
:::
...
The participant marks one point between a pair of opposing adjectives.
...
* **The pair must be genuinely opposite in the participant's language.** Pairs that feel neatly opposed in English often have no clean counterpart in another language.
...
Test your adjective pairs on a few prospective participants before the pilot. Ask one simple question: *"What would you say is the opposite of this word?"* If the answer is not the word you paired it with, the pair needs replacing.
:::
...
* In the right-hand panel, three participants deviate: they endorse a harder item while rejecting an easier one.
...
* Take two items that "might" be ordered:
  + **Item 1**: *"Being active in student organisations shows good time-management skills."*
  + **Item 2**: *"Good time-management skills do not come at the cost of academic achievement."*

* Under Guttman logic, agreeing with item 1 should mean agreeing with item 2 as well.

* But a participant might believe that being active in student organisations does build time-management skills while **also** costing academic achievement — a pattern that breaks the Guttman order.

* **The assumption that all items relate equally strongly to the latent variable does not hold** for Guttman items.
  + So the *parallel* and *tau-equivalent* models from Session 3 **do not fit** a Guttman scale, and coefficients such as Cronbach's $\alpha$ cannot be used for one.
...
* **Likert** assumes equal intervals between categories, that all participants read the anchors the same way, and that all items carry equal weight.
...
# Exercise {background-color="#14497F" .center}

## Find the problem in the items below

Each of the three response formats below has a problem.
...
different people. C: double-barrelled (competent and caring are two things); a participant who agrees
with only one half has no correct answer available.
...
5. **Semantic Differential** requires adjective pairs that are genuinely opposed in the participant's language.
...
* ***Forced-choice*** — when social desirability is the main threat, and participants must choose between options that are equally attractive.
