...
1. Reliabilitas terdiri atas **tiga pertanyaan/sumber *error* yang berbeda**
2. *Split-half* dan rumus **Spearman-Brown** 
3. ***Cronbach's alpha***: apa yang sebenarnya diukurnya
4. Demonstrasi dengan data riil: **$\alpha$ tidak bisa mendeteksi multidimensionalitas**
5. Koefisien **$\omega$** dan asumsi *tau-equivalent* dari Pertemuan 3
6. Menghitungnya di `jamovi`, dan apa yang sebaiknya Anda laporkan


# Definisi dan estimasinya {background-color="#14497F" .center}
...
* Masalahnya, $\sigma^2_T$ **tidak bisa kita amati**. Jadi rumus ini tidak bisa langsung diimplementasikan.

* Seluruh isi pertemuan hari ini adalah tentang **cara mengestimasinya** secara tidak langsung dari sesuatu yang bisa kita amati, yaitu **korelasi antar-item**.
...
Semua angka pada slide-slide berikut dihitung dari [`data/data.csv`](https://rameliaz.github.io/psp/data/data.csv), yang terdiri atas 50 item IPIP *Big Five*, **n = 19.718**. Anda bisa menghitung ulang semuanya sendiri di `jamovi`.
:::
...
* **Jarak waktunya sangat menentukan.** Terlalu dekat, partisipan masih ingat jawabannya. Terlalu jauh, atribut ukurnya mungkin memang sudah berubah secara alamiah.

* Masalahnya, kalau korelasinya rendah, kita **tidak bisa membedakan** apakah alat ukurnya tidak reliabel atau orangnya memang berubah.
...
* Kalau kedua bagian mengukur hal yang sama, korelasinya seharusnya tinggi.
...
* Korelasi dari teknik *split-half* ini adalah reliabilitas **tes sepanjang 5 item**, bukan 10 item.

* Kita baru saja memotong skala kita menjadi separuhnya, lalu melaporkan reliabilitas versi separuh dari keseluruhan skala.
...
Untuk menangani masalah ini, pembuat skala akan sangat terbantu kalau bisa menghitung berapa **kira-kira** reliabilitas suatu skala kalau jumlah itemnya dilipatgandakan dua kali. 
:::
...
::: {.callout-note appearance="simple"}
Rumus ini diturunkan secara terpisah oleh [**Spearman (1910)**](https://www.proquest.com/openview/9e46330dba2285dea63dd35588155383/1?pq-origsite=gscholar&cbl=1818401) dan [**Brown (1910)**](https://jhanley.biostat.mcgill.ca/bios601/Surveys/1910-brownReliability.pdf) pada tahun dan jurnal yang sama, sehingga namanya digabung.
:::
...
::: {.callout-warning}
#### Tetapi...
Rumus ini mengasumsikan item tambahannya **setara** dengan item yang sudah ada. Padahal kenyataannya, menambah sepuluh item yang ditulis asal-asalan justru akan membuat reliabilitas menjadi lebih buruk.
:::
...
* Skala 10 item bisa dibelah menjadi dua bagian dengan **126 cara berbeda** (ingat operasi kombinasi di matematika dasar!). 
  + Membagi skala menjadi dua belahan, masing-masing terdiri atas item nomor ganjil-genap hanyalah salah satunya.

* Setelah dicoba **500 pembelahan acak** pada skala Extraversion. Hasil Spearman-Brown-nya berkisar dari **0.845 sampai 0.920**.

* Jadi jawabannya bergantung pada belahan mana yang kebetulan Anda pilih. 
  + Ini tentu bukan situasi yang ideal.
...
Inilah ide dasar rumus Cronbach's $\alpha$.
:::
...
$\alpha$ adalah **rata-rata dari seluruh kemungkinan *split-half*** pada skala Anda ([Cronbach, 1951](https://www.cambridge.org/core/journals/psychometrika/article/abs/coefficient-alpha-and-the-internal-structure-of-tests/81D0CB193FA731FF5220FEB678FC4FAA)).
:::
...
## Mengapa reliabilitas harus setidaknya 0.70?
...
::: {.callout-warning appearance="simple"}
Jangan memperlakukan 0.70 sebagai *threshold*, tetapi **sesuaikan dengan tujuan penggunaan skor Anda**. Untuk penelitian kelompok, 0.70 memang memadai; untuk keputusan tentang individu utamanya ketika konteksnya *high-stake*, 0.7 masih jauh dari cukup.
:::
...
* Saya mengambil **5 item Extraversion** (E1, E2, E3, E5, E8) dan **5 item Neuroticism** (N1, N2, N7, N8, N9) dari data yang sama.

* Keduanya jelas mengukur konstruk yang **berbeda**. Skala gabungan ini secara substantif tidak masuk akal.

* Lalu saya memperlakukannya sebagai satu skala dengan 10 item dan menghitung $\alpha$.

* [Lihat demonstrasinya di sini.](/scripts/omega-demo.qmd)
...
* Angka 0.616 **tidak terlihat bermasalah**. Ia terlihat seperti skala yang tidak terlalu problematik dan masih bisa diperbaiki.

* Banyak laporan penelitian akan menuliskannya sebagai "cukup memadai" dan melanjutkan analisis.

* Padahal skala ini jelas-jelas **mencampur dua konstruk yang berbeda**.
...
| | Rata-rata korelasi |
|---|:--:|
| Antar-item di dalam subskala Extraversion | 0.436 |
| Antar-item di dalam subskala Neuroticism | 0.456 |
| **Antar-subskala** | **−0.111** |

. . .

::: {.incremental}

* Di dalam tiap subskala, korelasinya tinggi. Hal ini sudah cukup untuk mengangkat $\alpha$ ke 0.616.

* Korelasi antar-subskala negatif dan kecil, yang merupakan tanda bahwa keduanya bukan satu konstruk yang sama.

* *Eigenvalue* matriks korelasinya: **3.39 — 2.28 — 0.89**. Dua yang pertama sama-sama besar, artinya ada **dua** dimensi, bukan satu.

:::

## Sekarang hitung $\omega_h$

::: {.r-fit-text}
$\alpha = 0{.}616$ tetapi $\omega_h \approx 0{.}00$
:::
...
* $\omega$ *hierarchical* menanyakan hal yang berbeda dibandingkan $\alpha$: **berapa proporsi varians skor total yang berasal dari satu faktor laten yang general?**
...
* Kesimpulannya
  ...
  + $\omega_h \approx 0.00$ artinya *"item-item ini **tidak** mengukur satu konstruk yang sama"*
...
Dihitung dengan `psych::omega()` di {{< fa brands r-project >}}. Alternatifnya, Anda bisa menggunakan [*online calculator* dari Lensym](https://lensym.com/tools/mcdonalds-omega-calculator/).
:::
...
* Ingat dari Pertemuan 3: $\alpha$ mengasumsikan model ***tau-equivalent***, yaitu semua item punya hubungan yang **sama kuat** dengan variabel laten ($\theta$).

* Kalau item Anda sebenarnya *congeneric* (dan hampir selalu seperti ini), asumsi ini kemungkinan besar tidak terpenuhi.

* $\omega$ tidak mensyaratkan asumsi tersebut. Perhitungannya memakai korelasi antara masing-masing item dengan faktor laten ($\theta$).
...
* *Loading*-nya cukup seragam (0.55 sampai 0.76), sehingga pelanggaran asumsi *tau-equivalent* di sini kemungkinannya **kecil**.
...
* **Ketika skalanya multidimensional.** Di sini $\alpha$ tetap terlihat normal, sementara $\omega$ sangat rendah. Jadi, persis seperti demonstrasi tadi.
...
## Ragam definisi $\omega$

::: {.incremental}

* **McDonald's $\omega$ (versi asli, 1999)** — dihitung dari **satu** faktor laten (model *congeneric*). Formula yang barusan kita pakai: $(\sum\lambda_i)^2/[(\sum\lambda_i)^2+\sum\psi_i]$. 
  + Cocok digunakan kalau skala Anda memang unidimensional dan ini bisa dihitung dengan `jamovi`.

* Ketika [**Zinbarg, Revelle, Yovel, & Li (2005)**](https://link.springer.com/article/10.1007/s11336-003-0974-7) memperluasnya untuk skala dengan **satu faktor umum + beberapa faktor kelompok** (model *bifactor*), muncul dua angka terpisah:
  + **$\omega$ *hierarchical*** ($\omega_h$) — proporsi varians dari faktor umum **saja**.
  + **$\omega$ *total*** ($\omega_t$) — proporsi varians dari faktor umum **+ semua faktor kelompok**.

* Kalau skala Anda benar-benar unidimensional, ketiganya akan **sama persis**, tidak ada faktor perbedaan jumlah dimensi.
...
::: {.callout-warning appearance="simple"}
`jamovi`'s "McDonald's ω" pada *Reliability Analysis* **bukan** $\omega_h$. Ia dihitung dari model satu-faktor saja, tanpa dekomposisi *bifactor* sehingga tidak bisa mendeteksi multidimensionalitas seperti yang barusan kita tunjukkan lewat `psych::omega()` di R.
:::

## Membandingkan koefisiennya

| Koefisien | Model yang diasumsikan | Mengukur apa | Bisa dihitung di |
|---|---|---|---|
| Cronbach's $\alpha$ | *tau-equivalent* (semua item sama kuat) | rata-rata konsistensi antar-item | `jamovi`, hampir semua software |
| McDonald's $\omega$ | *congeneric*, satu faktor | varians dari satu faktor itu | `jamovi` ("McDonald's ω") |
| $\omega$ *hierarchical* ($\omega_h$) | *bifactor*: 1 faktor umum + faktor kelompok | varians dari faktor **umum saja** | `psych::omega()` di R |
| $\omega$ *total* ($\omega_t$) | *bifactor*: 1 faktor umum + faktor kelompok | varians dari faktor umum **+ kelompok** | `psych::omega()` di R |
...
::: {.callout-tip appearance="simple"}
Untuk demonstrasi tadi ($\alpha = 0{.}616$): $\omega$ versi `jamovi` akan tetap terlihat oke, tapi $\omega_h$ yang sesungguhnya **mendekati nol**. Kalau Anda mencurigai skala Anda multidimensional, jangan berhenti di angka yang ditampilkan `jamovi`, hitung $\omega_h$ lewat `psych::omega()` di R.

*Catatan teknis:* dekomposisi *bifactor* idealnya butuh **≥3 faktor kelompok** untuk identifikasi yang stabil. Dengan hanya 2 subskala seperti contoh kita, `psych` akan memberi peringatan ("*proceed with caution*"). Angkanya tetap informatif, tapi interpretasikan sebagai indikasi, bukan estimasi yang akurat.
:::
...
::: {.callout-warning appearance="simple"}
Jangan lupa membalik item yang diskor terbalik. Kalau tidak dilakukan, $\alpha$ Anda akan turun drastis dan Anda akan mengira skalanya bermasalah. Padahal yang bermasalah adalah penskorannya.

Tandanya sangat mudah dikenali: **korelasi item-rest bernilai negatif**.
:::
...
::: {.callout-note}
Pada set campuran tadi, *eigenvalue*-nya 3.39 dan 2.28, artinya keduanya besar. Inilah yang tidak bisa dilihat dari $\alpha = 0{.}616$.
:::
...
* ***Standard error of measurement* (SEM)**, kalau skor Anda akan dipakai untuk menafsirkan individu.
...
# Aktivitas {background-color="#14497F" .center}

## Empat situasi

Perhatikan empat situasi di bawah: **apa yang Anda simpulkan, dan apa langkah berikutnya?**

::: {.incremental}

**A.** Skala 30 item, $\alpha = 0{.}95$, rata-rata korelasi antar-item 0.42. Peneliti menyimpulkan reliabilitas skalanya sangat baik.

**B.** Skala 10 item, $\alpha = 0{.}62$, $\omega = 0{.}20$, ada dua nilai *eigenvalue* yang besar.

**C.** Skala 4 item, $\alpha = 0{.}55$. Peneliti ingin menaikkannya sampai di atas 0.70.
...
redundan (pelanggaran local independence dari Pertemuan 2), dan apakah beban partisipan sepadan.
...
1. Dari keempat situasi tadi, mana yang **paling bermasalah**? Kenapa?
...
3. Skor kelompok Anda akan dipakai untuk apa? **Menggambarkan kelompok** atau **menafsirkan individu**? Bagaimana itu mengubah standar reliabilitas yang Anda perlu pikirkan?
...
4. **$\alpha$ yang tinggi bisa diusahakan dengan menambah item**, dan **$\alpha$ tidak bisa mendeteksi multidimensionalitas** — $\alpha = 0{.}62$ vs. $\omega_h \approx 0$ adalah buktinya.
...
Kita masuk ke **validitas** dan pertanyaan soal validitas jauh lebih sulit dijawab.
...
::: {.callout-tip}
#### Persiapan
Baca **DeVellis & Thorpe (2022), Bab 4**. Kalau sempat, bab baca *Psychometric Validity* pada [Bikos](https://lhbikos.github.io/ReC_Psychometrics/).
:::
