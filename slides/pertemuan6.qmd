...
* Reliabilitas bukan satu hal: **tiga pertanyaan yang berbeda**
* *Split-half* dan rumus **Spearman-Brown** yang sudah dua kali kita tunda
* ***Cronbach's alpha***: apa yang sebenarnya diukurnya, dan apa yang tidak
* Demonstrasi dengan data nyata: **$\alpha$ tidak bisa mendeteksi multidimensionalitas**
...
* Seluruh isi pertemuan hari ini adalah tentang **cara menaksirnya** dari sesuatu yang bisa kita amati: korelasi.
...
Semua angka pada slide-slide berikut dihitung dari `data/data.csv` di repositori mata kuliah ini — 50 item IPIP *Big Five*, **n = 19.718**. Anda bisa mengulanginya sendiri di jamovi.
:::

# Reliabilitas bukan satu hal {background-color="#14497F" .center}
...
| Apakah skornya setara **lintas bentuk tes**? | Pemilihan item | *Alternate forms* |
| Apakah item-itemnya **saling konsisten**? | Keragaman antar-item | *Split-half*, $\alpha$, $\omega$ |

. . .

::: {.callout-warning}
#### Ini bukan tiga cara menghitung hal yang sama
Ketiganya memeriksa **sumber *error* yang berbeda**. Sebuah skala bisa sangat konsisten secara internal tetapi sama sekali tidak stabil dari minggu ke minggu.

Melaporkan $\alpha$ **tidak mengatakan apa pun** tentang stabilitas skor Anda lintas waktu.
:::
...
* **Jarak waktunya menentukan segalanya.** Terlalu dekat, responden masih ingat jawabannya. Terlalu jauh, atributnya mungkin memang sudah berubah.

* Ada persoalan yang tidak bisa dipecahkan: kalau korelasinya rendah, kita **tidak bisa membedakan** apakah alat ukurnya tidak reliabel atau orangnya memang berubah.
...
::: {.callout-note}
#### Untuk proyek Anda
*Test-retest* menuntut Anda menemui responden yang sama dua kali. Dengan jadwal semester ini, itu **tidak realistis**.

Yang penting: Anda tahu bahwa konsistensi internal yang akan Anda hitung **tidak menggantikan** informasi ini.
:::
...
Pada skala Extraversion IPIP (10 item), korelasi antara belahan ganjil dan genap adalah:
...
* Kita baru saja memotong skala kita menjadi separuhnya, lalu melaporkan reliabilitas versi separuh itu.

* Ingat dari Pertemuan 3: makin sedikit item, makin besar porsi *error*. Jadi angka 0,748 **meremehkan** reliabilitas skala yang sebenarnya.
...
::: {.callout-tip}
#### Yang dibutuhkan
Sebuah rumus yang menjawab: *"kalau tes sepanjang ini punya reliabilitas sekian, berapa reliabilitasnya kalau panjangnya dilipatgandakan?"*
:::

## Rumus Spearman-Brown

Untuk *split-half*, koreksi dari separuh ke panjang penuh:
...
Inilah rumus yang **dua kali kita tunda** — di Pertemuan 1 ketika membahas anak panah Bernoulli, dan di Pertemuan 3 ketika membahas kenapa menambah item menaikkan reliabilitas.

. . .

::: {.callout-warning}
#### Syarat yang sering dilupakan
Rumus ini mengandaikan item tambahannya **setara** dengan item yang sudah ada. Menambah sepuluh item asal-asalan tidak akan memberi hasil seperti ini.
:::

## Tapi belahan yang mana?
...
* Jadi jawabannya bergantung pada belahan mana yang kebetulan Anda pilih. Itu jelas tidak memuaskan.
...
Itulah yang dilakukan Cronbach's alpha.
:::
...
* Perhatikan apa yang dibandingkan: **jumlah varians item** melawan **varians skor total**.
...
## Buktinya: $\alpha$ adalah rata-rata semua pembelahan
...
::: {.callout-note}
#### Inilah arti $\alpha$ sesungguhnya
$\alpha$ bukan koefisien yang ajaib. Ia adalah **rata-rata dari seluruh kemungkinan *split-half*** pada skala Anda (Cronbach, 1951).

Itu sebabnya ia menyelesaikan masalah "belahan yang mana": ia memakai semuanya.
:::
...
* Perhatikan kurva paling bawah: dengan $\bar{r} = 0{,}10$, Anda butuh **lebih dari 35 item** untuk menembus 0,80.
...
::: {.callout-warning}
#### Konsekuensi yang tidak nyaman
Karena $k$ ikut menentukan, **$\alpha$ yang tinggi bisa dibeli hanya dengan menambah item**. Anda tidak perlu item yang lebih baik; cukup item yang lebih banyak.
:::

## Proyeksi Spearman-Brown pada skala kita

Berangkat dari $\alpha = 0{,}892$ pada 10 item:
...
* Menggandakan panjang skala menaikkan $\alpha$ sebesar 0,05. Menggandakannya lagi hanya menambah 0,018.

* **Hasilnya makin lama makin kecil**, sementara beban responden terus bertambah.
...
* Lance, Butts, & Michels (2006) memeriksa sumber aslinya. Nunnally menyebut 0,70 sebagai batas untuk **penelitian tahap awal** — untuk pengambilan keputusan tentang individu, ia menyarankan **0,90 atau lebih**.

* Angka 0,70 yang dipakai di mana-mana sebenarnya **kutipan yang salah tempat**.
...
Jangan memperlakukan 0,70 sebagai lulus-tidak lulus. **Sesuaikan dengan keperluan skor Anda**: untuk penelitian kelompok, 0,70 memang memadai; untuk keputusan tentang individu, jauh dari cukup.

Ingat perhitungan SEM di Pertemuan 3 — itulah cara memeriksanya secara konkret.
:::
...
* Angka 0,616 **tidak terlihat seperti bencana**. Ia terlihat seperti skala yang sedang-sedang saja dan masih bisa diperbaiki.

* Banyak laporan akan menuliskannya sebagai "cukup memadai" dan melanjutkan analisis.

* Padahal skala itu **mencampur dua konstruk yang berbeda**, dan kita tahu persis bahwa ia mencampurnya, karena kita sendiri yang menyusunnya.
...
| | Rata-rata korelasi |
|---|:--:|
| Antar-item di dalam blok Extraversion | 0,507 |
| Antar-item di dalam blok Neuroticism | 0,421 |
| **Antar-blok** | **−0,125** |
...
* Di dalam tiap blok, korelasinya tinggi. Itu sudah cukup untuk mengangkat $\alpha$ ke 0,616.

* Korelasi antar-blok negatif dan kecil — tanda bahwa keduanya bukan satu hal.

* Nilai *eigen* matriks korelasinya: **3,53 — 2,23 — 0,79**. Dua nilai pertama besar, artinya ada **dua** dimensi, bukan satu.
...
* $\omega$ menanyakan hal yang berbeda: **berapa proporsi varians skor total yang berasal dari satu faktor umum?**

* Pada set campuran ini, jawabannya nyaris tidak ada — karena memang tidak ada satu faktor umum di sana.

* Perbedaan antara 0,616 dan 0,151 adalah perbedaan antara *"item-item ini saling berkorelasi"* dan *"item-item ini mengukur satu hal yang sama"*.

:::

# Omega {background-color="#14497F" .center}

## Kenapa $\alpha$ butuh pendamping
...
* Kalau item Anda sebenarnya *congeneric* — dan hampir selalu begitu — asumsi itu tidak terpenuhi.

* $\omega$ tidak menuntut asumsi tersebut. Ia memakai *loading* tiap item apa adanya.
...
* Keduanya **hampir sama**. Ini bukan kebetulan.
...
::: {.callout-note}
#### Jangan salah paham
$\omega$ **bukan** selalu lebih besar atau lebih kecil dari $\alpha$. Ketika itemnya seragam, keduanya berdekatan. Perbedaannya muncul justru ketika ada sesuatu yang perlu Anda ketahui.
:::
...
* **Ketika *loading* item sangat beragam.** Di sini $\alpha$ cenderung lebih rendah daripada reliabilitas yang sesungguhnya.

* **Ketika skalanya multidimensional.** Di sini $\alpha$ tetap terlihat wajar, sementara $\omega$ jatuh — persis seperti demonstrasi tadi.
...
Hitung **keduanya**. Kalau $\alpha$ dan $\omega$ berdekatan, Anda punya bukti tambahan bahwa struktur skala Anda cukup rapi. Kalau jaraknya jauh, itu **pertanda yang harus Anda selidiki**, bukan angka yang tinggal dipilih salah satunya.
:::

## Tetapi keduanya tidak menggantikan pemeriksaan dimensi

::: {.incremental}

* Baik $\alpha$ maupun $\omega$ adalah **satu angka** yang meringkas keseluruhan skala.

* Satu angka tidak akan pernah memberi tahu Anda **ada berapa dimensi** di dalam item Anda.

* Untuk itu Anda perlu melihat struktur korelasinya sendiri: nilai *eigen*, *scree plot*, atau analisis faktor.
...
::: {.callout-warning}
Di Pertemuan 13 Anda akan melakukan ini pada data uji coba kelompok Anda. Urutannya penting: **periksa dimensinya dulu, baru hitung koefisien reliabilitasnya** — untuk tiap dimensi secara terpisah.
:::

# Ketepatan pada tingkat individu {background-color="#14497F" .center}

## SEM dengan angka yang nyata
...
Rentang kepercayaan 95% untuk seseorang yang mendapat skor **30**:
...
::: {.callout-warning}
#### Bacalah angka ini baik-baik
Ini adalah skala dengan $\alpha = 0{,}89$ — tergolong sangat baik menurut kriteria mana pun. Namun rentang kepercayaan satu orang **selebar hampir 12 poin** pada skala yang rentangnya 10 sampai 50.

Dua orang dengan skor 30 dan 35 **tidak bisa** Anda katakan berbeda.
:::
...
* $\alpha$ yang tinggi **tidak** membebaskan Anda dari persoalan ini.
...
Lupa membalik item yang terbalik. Kalau itu terjadi, $\alpha$ Anda akan anjlok dan Anda akan mengira skalanya rusak — padahal yang rusak adalah penskorannya.

Gejalanya khas: **korelasi item-rest bernilai negatif**.
:::
...
Jangan membuang item hanya karena angkanya. Kalau sebuah item **penting secara konseptual** untuk mendefinisikan konstruk Anda, membuangnya demi menaikkan α berarti Anda menukar validitas isi dengan angka yang lebih cantik.

Kita bahas persoalan ini lebih jauh di Pertemuan 7 dan Pertemuan 11.
:::

## Memeriksa dimensinya

::: {.incremental}

* **Analyses → Factor → Exploratory Factor Analysis**, lalu perhatikan **eigenvalues** dan **scree plot**.

* Kalau hanya ada satu nilai *eigen* yang jauh lebih besar dari sisanya, skala Anda kemungkinan satu dimensi.
...
::: {.callout-note}
Pada set campuran tadi, nilai *eigen*-nya 3,53 dan 2,23 — dua-duanya besar. Itulah yang tidak bisa dilihat dari $\alpha = 0{,}616$.
:::
...
* **Jumlah item** yang dihitung, dan **item mana yang dibalik**.

* **Ukuran dan karakteristik sampel** — ingat dari Pertemuan 3 bahwa koefisien ini bergantung pada sampel.

* **Bukti tentang dimensinya**, sekurang-kurangnya nilai *eigen* atau *scree plot*.
...
::: {.callout-tip}
#### Satu kalimat yang baik terlihat seperti ini
...
**B.** Skala 10 item, $\alpha = 0{,}62$, $\omega = 0{,}20$, dua nilai *eigen* besar.
...
1. Dari keempat situasi tadi, mana yang **paling berbahaya** kalau lolos ke laporan akhir? Kenapa?
...
::: {.callout-tip}
Pertanyaan nomor 3 menentukan segalanya, dan sering tidak pernah ditanyakan.
:::
...
1. **Reliabilitas bukan satu hal.** Stabilitas lintas waktu dan konsistensi internal adalah pertanyaan yang berbeda.

2. **Spearman-Brown** mengoreksi *split-half* ke panjang penuh, dan menjelaskan kenapa menambah item menaikkan reliabilitas.

3. **$\alpha$ adalah rata-rata dari seluruh kemungkinan *split-half***, dan hanya ditentukan oleh $k$ dan $\bar{r}$.

4. **$\alpha$ yang tinggi bisa dibeli dengan menambah item**, dan **$\alpha$ tidak bisa mendeteksi multidimensionalitas** — $\alpha = 0{,}62$ dengan $\omega = 0{,}15$ adalah buktinya.

5. **Hitung $\alpha$ dan $\omega$ bersama-sama**, dan periksa dimensinya secara terpisah.

6. **Ambang 0,70 bukan hukum.** Standarnya ditentukan oleh untuk apa skor itu dipakai.
...
* Kenapa validitas adalah sifat **tafsiran atas skor**, bukan sifat skala. Kita sudah menyinggungnya di Pertemuan 3; minggu depan kita bahas penuh.
...
* Kenapa reliabilitas tinggi **tidak menjamin apa pun** tentang validitas.
