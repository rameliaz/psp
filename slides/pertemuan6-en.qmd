...
* **McDonald's $\omega$ (the original, 1999)** — computed from a **single** latent factor (a congeneric model).
  + Appropriate when your scale really is unidimensional, and this is what `jamovi` computes.

* [**Zinbarg, Revelle, Yovel, & Li (2005)**](https://link.springer.com/article/10.1007/s11336-003-0974-7) extended it to scales with **one general factor plus several group factors** (a bifactor model):
...
::: {.callout-warning appearance="simple"}
McDonald's ω in `jamovi` is **not** $\omega_h$. It comes from a single-factor model with no bifactor decomposition, so it cannot detect multidimensionality the way we just demonstrated with `psych::omega()` in {{< fa brands r-project >}}.
:::

## Comparing the coefficients {.smaller}
...
| $\omega$ *hierarchical* ($\omega_h$) | *bifactor*: 1 general factor + group factors | variance from the **general factor alone** | `psych::omega()` in {{< fa brands r-project >}} or an [online calculator](https://lensym.com/tools/mcdonalds-omega-calculator/) |
| $\omega$ *total* ($\omega_t$) | *bifactor*: 1 general factor + group factors | variance from the general factor **plus group factors** | `psych::omega()` in {{< fa brands r-project >}} or an [online calculator](https://lensym.com/tools/mcdonalds-omega-calculator/) |
...
* For that you have to look at the correlation structure itself: eigenvalues, a scree plot, or a [factor analysis](https://rameliaz.github.io/statistik-lanjut/slides/bagian-7.html#/title-slide).
