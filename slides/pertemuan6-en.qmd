...
1. Reliability is not one thing: **three different questions**
2. *Split-half* and the **Spearman-Brown** formula
3. ***Cronbach's alpha***: what it actually measures, and what it does not
4. A demonstration with real data: **$\alpha$ cannot detect multidimensionality**
5. The ***omega*** coefficient and the tau-equivalence assumption from Session 3
6. Computing it in **jamovi**, and what you should report
...
Every number on the following slides is computed from [`data/data.csv`](https://rameliaz.github.io/psp/data/data.csv) — 50 IPIP *Big Five* items, **n = 19,718**. You can reproduce all of it in jamovi.
:::

# Reliability {background-color="#14497F" .center}
...
* **The interval matters a great deal.** Too short and participants remember their answers; too long and the attribute may genuinely have changed.
...
::: {.callout-note appearance="simple"}
The formula was derived independently by [**Spearman (1910)**](https://www.proquest.com/openview/9e46330dba2285dea63dd35588155383/1?pq-origsite=gscholar&cbl=1818401) and [**Brown (1910)**](https://jhanley.biostat.mcgill.ca/bios601/Surveys/1910-brownReliability.pdf) in the same year and the same journal, which is why both names are attached.
:::
...
* A 10-item scale can be split into halves in **126 different ways** (recall combinations from basic math!).
  + Odd-even is only one of them.

* We tried **500 random splits** of the Extraversion scale. The Spearman-Brown results ranged from **0.845 to 0.920**.

* So the answer depends on which split you happen to choose.
...
$\alpha$ is **the average of all possible split-halves** of your scale ([Cronbach, 1951](https://www.cambridge.org/core/journals/psychometrika/article/abs/coefficient-alpha-and-the-internal-structure-of-tests/81D0CB193FA731FF5220FEB678FC4FAA)).
:::
...
* **The returns keep shrinking** while participant burden keeps growing.
...
* [Lance, Butts, & Michels (2006)](https://journals.sagepub.com/doi/abs/10.1177/1094428105284919) went back to the source. Nunnally proposed .70 only for **the early stages of research**; for basic research he recommended **.80**, and for decisions about individuals **.90 or higher**.
...
* We took **5 Extraversion items** (E1, E2, E3, E5, E8) and **5 Neuroticism items** (N1, N2, N7, N8, N9) from the same dataset.

* They clearly measure **different** constructs, so the combined scale makes no substantive sense.

* We then treated them as a single 10-item scale and computed $\alpha$.

* [See the demonstration here.](/scripts/omega-demo.qmd)
...
* Yet that scale **mixes two different constructs**.
...
| | Mean correlation |
|---|:--:|
| Between items within the Extraversion block | .436 |
| Between items within the Neuroticism block | .456 |
| **Between blocks** | **−.111** |
...
* The eigenvalues of the correlation matrix: **3.39, 2.28, 0.89**. The first two are both large, meaning **two** dimensions, not one.
...
::: {.r-fit-text}
$\alpha = 0.616$ but $\omega_h \approx 0.00$
:::
...
* $\omega$ *hierarchical* asks a different question from $\alpha$: **what proportion of total score variance comes from one general latent factor?**
...
* In short
  ...
  + $\omega_h \approx 0.00$ means *"these items do **not** measure the same construct"*
...
Computed with `psych::omega()` in {{< fa brands r-project >}}. Alternatively, you can use the [Lensym online calculator](https://lensym.com/tools/mcdonalds-omega-calculator/).
:::
...
::: {.callout-tip appearance="simple"}
Compute **both**. If $\alpha$ and $\omega$ are close, you have additional evidence that your scale's structure is unidimensional. If they are far apart, that is **a sign that something about the construct needs checking**.
:::
...
* **McDonald's $\omega$ (the original, 1999)** — computed from a **single** latent factor (a congeneric model). This is the formula we just used: $(\sum\lambda_i)^2/[(\sum\lambda_i)^2+\sum\psi_i]$.
  + Appropriate when your scale really is unidimensional, and this is what `jamovi` computes.

* When [**Zinbarg, Revelle, Yovel, & Li (2005)**](https://link.springer.com/article/10.1007/s11336-003-0974-7) extended it to scales with **one general factor plus several group factors** (a bifactor model), two separate numbers emerged:
  + **$\omega$ *hierarchical*** ($\omega_h$) — the proportion of variance from the general factor **alone**.
  + **$\omega$ *total*** ($\omega_t$) — the proportion of variance from the general factor **plus all the group factors**.

* If your scale is genuinely unidimensional, all three coincide **exactly** — there is no group-factor variance to separate out.
...
::: {.callout-warning appearance="simple"}
`jamovi`'s "McDonald's ω" in Reliability Analysis is **not** $\omega_h$. It comes from a single-factor model with no bifactor decomposition, so it cannot detect multidimensionality the way we just demonstrated with `psych::omega()` in R.
:::

## Comparing the coefficients

| Coefficient | Model assumed | What it measures | Computable in |
|---|---|---|---|
| Cronbach's $\alpha$ | *tau-equivalent* (all items equally strong) | average inter-item consistency | `jamovi`, almost any software |
| McDonald's $\omega$ | *congeneric*, one factor | variance from that one factor | `jamovi` ("McDonald's ω") |
| $\omega$ *hierarchical* ($\omega_h$) | *bifactor*: 1 general factor + group factors | variance from the **general factor alone** | `psych::omega()` in R |
| $\omega$ *total* ($\omega_t$) | *bifactor*: 1 general factor + group factors | variance from the general factor **plus group factors** | `psych::omega()` in R |
...
::: {.callout-tip appearance="simple"}
For our demonstration ($\alpha = 0.616$): `jamovi`'s version of $\omega$ would still look fine, but the true $\omega_h$ is **close to zero**. If you suspect your scale is multidimensional, do not stop at what `jamovi` displays — compute $\omega_h$ with `psych::omega()` in R.

*Technical note:* a bifactor decomposition ideally needs **≥3 group factors** for stable identification. With only 2 subscales, as in our example, `psych` will issue a warning ("proceed with caution"). The number is still informative, but treat it as an indication rather than a precise estimate.
:::
...
::: {.callout-warning appearance="simple"}
Do not forget to reverse the reverse-worded items. When that step is skipped, your $\alpha$ drops sharply and it looks like the scale is faulty — when what is actually faulty is the scoring.
...
::: {.callout-tip appearance="simple"}
Do not drop an item just because its item-rest correlation is low. If the item is **conceptually essential** to defining your construct, removing it to raise $\alpha$ means trading content validity for internal consistency.
:::
...
::: {.callout-note}
For the mixed set earlier, the eigenvalues were 3.39 and 2.28 — both large. That is precisely what $\alpha = .616$ could not reveal.
:::
...
# Activity {background-color="#14497F" .center}

## Four situations

Look at the four situations below: **what do you conclude, and what's the next step?**
...
(a local independence violation from Session 2) and whether the participant burden is justified.
...
1. Of the four situations, which is **most problematic**? Why?
...
4. **A high $\alpha$ can be bought by adding items**, and **$\alpha$ cannot detect multidimensionality** — $\alpha = .62$ against $\omega_h \approx 0$ is the proof.
