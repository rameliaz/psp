...
* **Assumption 6**: participants can and will report their internal states
...
# Recap of Session 1 {background-color="#14497F" .center}
...
| 6 | Participants **can and will** answer accurately | Partly |
...
* **Adolphe Quetelet** (1835) introduced *l'homme moyen*, "the average man".
  + He applied the normal curve — at the time a model of **observational error** in astronomy — to human characteristics.
...
* **Galton** then extended it to psychological traits, and normality has been built into psychometrics ever since.
  + This is what justified the deeply racist worldview of [*eugenics*](https://www.genome.gov/about-genomics/fact-sheets/Eugenics-and-Scientific-Racism).
...
::: {.callout-warning}
#### An inherited habit, not a finding
...
* In scale construction we **discard items** that are too easy or too hard to endorse — items that nearly everyone answers "5/strongly agree" (e.g., *"I believe God truly exists"*), or that nearly everyone answers "1/strongly disagree" (e.g., *"Premarital sex is a moral obligation everyone must fulfil"*).

* The items that survive are the ones that **split participants down the middle**.
...
IPIP *Big Five* data (`data/data.csv` in this course repository), 50 items, 1–5 scale, **n = 19,719** participants (the raw row count; Session 6 uses n = 19,718 after listwise deletion).
...
* And the distribution is still not quite normal: it is flatter, with participants spread more evenly across the middle than a normal curve would predict.
...
This is also the problem baked into diagnostic and classification systems for mental disorders: a categorical classification system applied to constructs whose evidence points to a continuum. It is also what motivated the [Research Domain Criteria (RDoC)](https://www.nimh.nih.gov/research/research-funded-by-nimh/rdoc) project.
:::
...
* Most psychological scales — including the one you are about to build — assume **dimensional**. That is a common assumption, but it is still one that needs to be tested.
  + That said, testing it is beyond the scope of the project you are doing in this course.
...
* In other words: any correlation between two items should trace entirely to the latent construct, **not** to anything else.
...
* We assume the same item **means the same thing** to men and women, to students and workers, to participants from different ethnic backgrounds (e.g., Javanese and Sulawesi participants).
...
* As a result, participants from a culture with a higher local standard may rate themselves **lower**, even when their behaviour fits the construct better.
...
* Back-translation ensures the **words** match. It does not ensure the **meaning** matches, still less that the item functions the same way for Indonesian participants.
...
# Assumption 6: participants can and will answer accurately {background-color="#14497F" .center}
...
1. Participants are **able** to observe their internal states and report them.

2. Participants are **willing** to report them honestly.
...
* Participants tend to answer in ways that make them look good — sometimes deliberately, often not.
...
If your construct has a socially "correct" answer, your score distribution will **pile up at one end** (e.g., a [*ceiling* or *floor effect*](https://matilda.fss.uu.nl/articles/floor-ceiling-effects.html)). You will struggle to distinguish participants from one another because your data will have low variance.
:::
...
But reverse-worded items bring their own problems: some participants misread them, and they often cluster together in factor analysis as though they formed a separate dimension. We weigh this up in Session 10.
:::
...
* Some participants complete a questionnaire without reading it — especially long, online, unincentivised ones.
...
* This is not only about participant attitude; it is a consequence of **instrument design** that overburdens people.
...
6. **Accurate participants** — people have limited access to themselves, and reasons not to be candid.
