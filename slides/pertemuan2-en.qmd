...
* Why assumptions need scrutiny, and why that does not mean we stop measuring
...
::: {.callout-note}
#### The common thread
Every one of those points has the same shape: **something we take to be true without ever checking it**. Today we check them one at a time.
:::
...
# What is an assumption, and why check it {background-color="#14497F" .center}
...
3. **The assumption is violated, and the conclusion changes completely.**
...
::: {.callout-warning}
#### Our job
To tell case 2 from case 3. This **cannot** be decided from the data alone — it requires understanding the construct being measured.
:::
...
::: {.callout-note}
#### The good news
This is precisely what separates *Classical Test Theory* from *Item Response Theory*. IRT lets each item have its own difficulty and discrimination.
...
* The shift is subtle but large: the normal curve moved from being **a model of error** to being **a model of people**.
...
IPIP *Big Five* data (`data/data.csv` in this course repository), 50 items, 1–5 scale, **n = 19,718** respondents.
...
# Assumption 3: the attribute is continuous {background-color="#14497F" .center}
...
* **Haslam, Holland, & Kuppens (2012)** reviewed 177 taxometric studies of personality and psychopathology constructs.

* Their conclusion: **most constructs appear dimensional**. Evidence for natural categories was found for only a few.
...
This is exactly the tension we saw in the DSM: a categorical classification system applied to constructs whose evidence points to a continuum. It is also what motivated the RDoC project.
:::
...
* Most psychological scales — including the one you are about to build — assume **dimensional**. That is a reasonable assumption, but it is still an assumption.
...
A single sentence is enough: *"We treat this construct as dimensional, on the grounds that ..."* — rather than simply doing it without comment.
:::
...
::: {.callout-note}
#### A consequence you will use constantly
Because of this, inter-item correlations can be used to **estimate** how strongly each item relates to its latent construct — even though we never measure the construct directly. This is the basis of item analysis in Session 13.
:::
...
If your construct has a socially "correct" answer, your score distribution will **pile up at one end** — and you will struggle to distinguish respondents from one another. This will show up immediately in item analysis in Session 13.
:::
...
::: {.callout-tip}
These five points will form part of your final report. Start recording your answers now, not in the last week.
:::
...
* Work in the **project group** you formed last week.

* Use the **construct your group chose** in Session 1.

* Discuss for **25 minutes**, then each group presents the **single** assumption it considers riskiest for its construct, in 3 minutes.

:::

::: {.notes}
If a group claims every assumption is safe for their construct, ask them to picture the two most
extreme respondents: the person who would score highest and the person who would score lowest.
The problematic assumption usually becomes obvious immediately.
Leave about 20 minutes for presentations and the wrap-up.
:::
...
1. **Of today's six assumptions, which is riskiest for your construct?** Explain why, rather than just naming it.

2. **Does your construct have a socially "correct" answer?** If so, how will that affect your score distribution?

3. **Name two subgroups in your target population who might read one item differently.** Write the item, and write both readings.

4. **Which of these assumptions can you genuinely check** with the pilot data you will realistically collect this semester? Which can you only argue for?
...
::: {.callout-tip}
#### Question 4 is the decisive one
Distinguishing assumptions you can **test** from assumptions you can only **defend by argument** is the core of honest psychometric work.
:::
...
5. **The same for every group** — an identical item does not always mean the same thing; this is not a translation problem.
