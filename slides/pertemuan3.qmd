...
1. Apa yang dimaksud dengan **variabel laten**, dan kenapa ia disebut sebagai *penyebab*
2. ***Classical Test Theory***: $X = T + E$ dan asumsi-asumsinya
3. Kenapa reliabilitas didefinisikan sebagai **proporsi varians** dari *true score* dan *error* pengukuran
4. Apa sebenarnya arti *true score* dan kenapa ia **bukan** "nilai konstruk yang sesungguhnya"
5. Konsekuensi CTT: jumlah item, atenuasi, dan ketergantungan pada sampel
6. Keluarga model dalam CTT: *parallel*, *tau-equivalent*, *congeneric*
7. Sekilas ***Item Response Theory*** dan apa *benefit* yang ditawarkannya


# Asumsi ➡️ Model Pengukuran {background-color="#14497F" .center}
...
* **Hari ini**: asumsi-asumsi itu kita "formalkan" menjadi sebuah **model matematika** agar dapat diuji secara empirik
...
Selama asumsi hanya berupa daftar, kita tidak bisa melakukan apapun untuk mengujinya. Begitu asumsinya dituliskan sebagai persamaan matematika, kita bisa **menguji konsekuensinya**.
:::

# Definisi variabel laten {background-color="#14497F" .center}

## Variabel laten vs. indikator
...
* ***Laten***, bukan manifes. 
  + Tidak bisa diamati langsung. Aspirasi orang tua terhadap prestasi anaknya tidak bisa dilihat, hanya bisa disimpulkan dari indikator-indikator yang merefleksikan aspirasi tsb.

* ***Variabel***, bukan konstanta. 
  + Ada sesuatu yang berubah — kekuatannya, besarannya — antar-orang, antar-waktu, antar-situasi.
...
![](libs/reflektif.png){fig-align="center" width="100%"}
...
::: {.callout-note appearance="simple"}
Prosedur yang akan Anda jalankan di Pertemuan 13: *corrected item-total correlation*, atau membuang item yang korelasinya lemah, seluruhnya bersandar pada logika ini.
:::
...
* $X$ — ***observed score***. Skor yang benar-benar Anda dapatkan dari partisipan.
...
::: {.callout-note}
#### Cakupan $E$
Kondisi fisik partisipan saat mengisi, suasana hati sesaat, ambiguitas kalimat item, gangguan di ruangan, salah klik/pilih jawaban. CTT **tidak memisahkan** sumber-sumber ini. Semuanya dikumpulkan menjadi satu suku *error*.
:::
...
Ketiganya menyatakan bahwa *error* bersifat **acak**, bukan sistematis. Bias yang konsisten, misalnya seluruh partisipan menjawab lebih positif karena *social desirability*, **tidak termasuk ke dalam** $E$.
:::

## Memecah varians skor total
...
* Artinya: **variasi skor yang kita amati** seharusnya terdiri atas variasi yang berasal dari perbedaan antar-orang yang sesungguhnya, ditambah variasi yang berasal dari *noise*.
...
Seseorang yang mendapat skor kasar **30**, maka skor sebenarnya berada di suatu rentang, bukan di satu titik. Dengan tingkat kepercayaan 95% (1.96, [lihat gambar ini](https://www.researchgate.net/profile/Avijit-Hazra-2/publication/320742650/figure/tbl1/AS:669269071237158@1536577589946/Critical-z-values-used-in-the-calculation-of-confidence-intervals.png)), maka:
...
* *True score* adalah "skor yang stabil yang dihasilkan oleh suatu skala", bukan "kadar konstruk yang sesungguhnya ada pada partisipan/partisipan".
...
Reliabilitas memberi **batas atas** bagi validitas, yaitu korelasi antara skor Anda dan suatu kriteria (e.g., *criterion-related validity*):

$$r_{XY} \leq \sqrt{\rho_{XX'} \times \rho_{YY'}}$$

Ini melibatkan reliabilitas kriterianya juga. Kalau kriteria diasumsikan diukur tanpa *error* ($\rho_{YY'}=1$), rumusnya menjadi lebih sederhana, $r_{XY} \leq \sqrt{\rho_{XX'}}$: dengan $\rho_{XX'} = 0{.}64$, korelasi maksimum yang mungkin Anda peroleh adalah $0{.}80$ — berapa pun kuatnya hubungan yang sesungguhnya. Kalau kriterianya sendiri juga *fallible* (mengandung *error*), batas atasnya turun lebih jauh.
:::
...
* Bukti validitas **tidak otomatis berpindah**. 
  + Skala yang tafsiran skornya terdukung untuk penelitian pada sampel mahasiswa belum tentu terdukung untuk seleksi kerja atau penegakan diagnosis.
...
## Hubungkan dengan Pertemuan 1️⃣ dan 2️⃣
...
* Dan kuesioner yang terlalu panjang memunculkan masalah baru: kelelahan partisipan dan jawaban yang asal-asalan.
...
* Jadi, semakin besar *error*, penyebut menjadi bertambah tanpa menambah pembilang. Efeknya, korelasi antara dua variabel ini mengecil.
...
* Kalau partisipan Anda sangat seragam pada atribut yang diukur, rentang *true score* mereka sempit. Korelasi antar-item ikut mengecil, dan **reliabilitas terhitung menjadi rendah** — padahal alat ukurnya sama saja.
...
Reliabilitas yang Anda laporkan **bukan sekadar sifat alat ukur Anda**. Ia juga sebagian menunjukkan karakteristik sampel Anda. Karena itu, selalu laporkan **karakteristik sampel** bersama koefisien reliabilitasnya.
:::
...
* **Tingkat kesulitan item** dalam CTT diukur sebagai proporsi partisipan yang menyetujuinya. Angka ini berubah kalau sampelnya berubah.
...
![](libs/icc-diskriminasi.svg){.nostretch fig-align="center" width="60%"}
...
4. **Kalau seluruh item Anda "mudah disetujui", apa yang akan terjadi** pada sebaran skor dan pada kemampuan skala Anda membedakan partisipan?
...
1. **Variabel laten adalah sebab**, dan inilah alasan mengapa item dalam satu skala harus saling berkorelasi.
...
4. ***True score* melekat pada alat ukur**, bukan pada konstruk. Alat ukur yang memiliki bias secara konsisten tetap bisa sangat reliabel. Reliabilitas maupun validitas sama-sama sifat **skor**, bukan sifat skala.

5. **Statistik item dalam CTT bergantung pada sampel.** Laporkan karakteristik sampel bersama koefisiennya.

6. **$\alpha$ mengasumsikan model *tau-equivalent***, yang jarang terpenuhi. Oleh karena itu, lebih baik gunakan $\omega$ yang mengasumsikan model *congeneric*.
...
* **Likert** — berapa pilihan respon yang sebaiknya dipakai, dan apakah perlu pilihan di tengah-tengah?
