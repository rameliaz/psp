...
* **Construct boundary**: what is excluded, and neighbouring constructs
...
* Today is not a lecture. Most of the time is **group work**, and we will move between groups.
...
Today's output fills two rows of the project rubric: **Introduction** and **Construct operationalisation**. It also underpins **Item writing**, since items cannot be judged without a clear construct definition.
:::
...
::: {.callout-warning}
Flake & Fried (2020) list "not reporting how the construct was defined" among their questionable measurement practices. This is not merely a matter of tidy writing.
:::
...
* You are **not** summarising all the research on the construct.
...
::: aside
Adapted from Podsakoff, MacKenzie, & Podsakoff (2016).
:::
...
* The level of specificity is a **decision**, not something that emerges on its own.
...
If your construct already has a name other people use, is your definition **the same** as theirs? If it differs, say that it differs — do not use the same name for different content.
:::
...
1. **Which quantity?** A person's level of the attribute? Their standing relative to others? The presence or absence of a condition?
...
::: {.callout-warning}
This statement will be tested against data in Session 13. If you claim three dimensions and the factor analysis shows one, that is **a finding to report**, not a failure to conceal.
:::
...
Work through these in order. We will come round to each group.
...
* **20 minutes** — Build the two lists: included and excluded. Add two neighbouring constructs and how yours differs.
...
2. **One neighbouring construct** and how yours differs from it.
...
::: {.callout-tip}
Other groups may ask one question: *"what makes you confident your items will not also measure that neighbouring construct?"*
:::
...
* **Neighbouring constructs** — at least two, with the distinctions.
...
::: {.callout-note}
This document becomes an appendix to your final report, and the material the expert panel reads in Session 11. The panel cannot judge item relevance without first reading the construct definition.
:::
