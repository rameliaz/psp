...
* Apa yang sebenarnya kita pilih ketika memilih format respons
* **Likert**: asal-usulnya, dan empat keputusan yang harus Anda ambil
...
* Pertanyaan penutup: **memilih format respons itu mengubah asumsi apa?**
...
* **Pertemuan 3** memberi kita modelnya: $X = T + E$, variabel laten sebagai sebab, dan reliabilitas sebagai proporsi varians.

* Sampai di sini semuanya masih abstrak. 

* **Format respons adalah tempat pertama** di mana teori pengukuran (yang abstrak) berubah menjadi sesuatu yang lebih *practical* dan terlihat di kuesioner.
...
* Sebagian besar item terdiri atas dua bagian: **stem** (pernyataan atau pertanyaan) dan **opsi respons**.

* Yang Anda pilih ketika memilih format sekaligus mencakup:
...
Satu item tunggal dengan lima pilihan **bukan** "skala Likert". Itu adalah **item bertipe Likert** (*Likert-type item*).
...
Keempatnya adalah keputusan yang terpisah. Mari kita bahas satu per satu.

## Keputusan 1: berapa pilihan respons?

::: {.incremental}

* **Di bawah 4 pilihan**, Anda kehilangan informasi. Variasi antar-responden tertekan, dan reliabilitas ikut turun (Preston & Colman, 2000).

* **Sekitar 5–7 pilihan** adalah wilayah yang paling sering direkomendasikan. Lozano dkk. (2008) menyarankan minimum 4, dengan hasil terbaik pada 6–7.

* **Di atas 6–7 pilihan**, tambahan keuntungannya kecil. Simms dkk. (2019) menemukan hampir tidak ada perbaikan psikometrik setelah 6 pilihan.
...
Pertama, **kemampuan responden membedakan**. Kalau responden tidak sanggup membedakan tujuh tingkat, tujuh pilihan hanya menambah *noise*.

Kedua, **kesediaan Anda memakai ketelitian itu**. Kalau nanti skornya Anda kelompokkan menjadi tiga kategori, buat apa Anda menggunakan 10 pilihan respons?
:::

## Keputusan 2: pakai titik tengah atau tidak?
...
**Memaksa** responden memihak, walau lemah.

Cocok kalau Anda curiga netral akan dipakai untuk menghindar.
...
* Kalau setiap item punya pilihan di tengah, sebagian responden akan belajar bahwa mereka **selalu bisa memilih pilihan itu** dan menyelesaikan kuesioner lebih cepat.
...
Ketika titik tengah tersedia, ia sering menjadi **opsi yang paling banyak dipilih**. Kalau itu terjadi pada data uji coba Anda di Pertemuan 12, jangan langsung menyimpulkan respondennya memang netral.
:::

## Netral dan ragu-ragu itu tidak sama
...
::: {.callout-warning}
#### Kenapa ini tidak bisa diremehkan
...
## Keputusan 3: melabeli semua pilihan, atau hanya ujungnya?

::: {.incremental}

* Melabeli **semua pilihan** umumnya menghasilkan reliabilitas dan validitas yang lebih baik daripada hanya melabeli kedua ujungnya (Krosnick & Presser, 2010).
...
* Tetapi label yang dipilih harus benar-benar bisa dibedakan oleh partisipan.
...
## Keputusan 4: *Anchor* apa yang dipakai?
...
::: {.callout-tip}
#### Kecenderungan yang perlu Anda tahu
*Anchor* persetujuan mendorong responden untuk sekadar mengiyakan. *Anchor* yang khusus untuk isi item umumnya menekan kecenderungan itu, karena responden harus benar-benar membaca pilihannya.

Weijters dkk. (2010) menunjukkan bahwa format skala ikut menentukan *response style* yang muncul.
:::
...
Kalau kalimatnya sendiri sudah sangat lunak (versi 3), hampir semua orang akan setuju, dan item itu tidak lagi membedakan siapa pun. Ingat pembahasan tingkat kesulitan item di Pertemuan 3.
:::
...
Responden menandai satu titik di antara sepasang kata sifat yang berlawanan.
...
* Ketika beban membaca perlu ditekan: pasangan kata sifat jauh lebih ringan daripada deretan kalimat panjang.
...
* Pada panel kanan, tiga responden menyimpang: ada yang menyetujui item yang sulit tetapi menolak item yang lebih mudah.
...
* Ketika hierarkinya merupakan **konsekuensi logis**: makan 10 mangkok bakso selalu berarti lebih banyak daripada 5 mangkok.
...
## Di mana Guttman tidak berguna sama sekali

::: {.incremental}

* Begitu konstruknya tidak konkret, urutannya **tidak lagi sama untuk semua orang**.

* DeVellis memberi contoh: dua item tentang aspirasi orang tua yang secara logika berjenjang, tetapi seseorang bisa saja menyetujui item ketiga dan menolak item keempat karena ia memandang kesuksesan sebagai hal yang sekaligus membantu dan menghambat kebahagiaan.
...
DeVellis mencatat satu hal yang mudah terlewat: **asumsi bahwa semua item punya hubungan yang sama kuat dengan variabel laten tidak berlaku** untuk item Guttman.

Artinya, model *parallel* dan *tau-equivalent* dari Pertemuan 3 **tidak cocok** untuk skala Guttman — dan koefisien seperti $\alpha$ tidak bisa dipakai untuk skala Guttman.
:::

## Skala Guttman masih berguna

::: {.incremental}

* Kelemahan Guttman ada pada sifatnya yang **deterministik**: pola yang menyimpang dianggap kesalahan.

* *Item Response Theory* mengambil gagasan yang sama tetapi membuatnya **probabilistik**: makin tinggi $\theta$ seseorang, makin **besar peluangnya** menyetujui item yang sulit, jadi bukan kepastian.

* Model Rasch pada dasarnya adalah **versi probabilistik dari gagasan Guttman**.
...
::: {.callout-tip}
Inilah alasan kurva karakteristik item di Pertemuan 3 berbentuk landai, bukan berupa tangga tegak. Guttman mengasumsikan tangga; IRT mengasumsikan kurva.
:::
...
|---|---|---|---|
| Bentuk item | Pernyataan + derajat setuju | Pasangan kata sifat | Pernyataan berjenjang, ya/tidak |
...
| Kelemahan utama | *Acquiescence*, jarak tak setara | Padanan antonim, ambiguitas tengah | Penyimpangan pola |

. . .

::: {.callout-note}
Untuk proyek Anda semester ini, **Likert dan Semantic Differential adalah pilihan yang realistis**. Guttman hanya masuk akal kalau konstruk Anda memang berjenjang secara teoretis.
:::
...
* **Guttman** mengasumsikan urutan kumulatif yang **deterministik** — asumsi paling keras di antara ketiganya.
...
Empat format respons berikut punya masalah. Diskusikan dalam kelompok, **15 menit**.
...
1. **Format respons bukan soal selera.** Setiap format membawa asumsinya sendiri.
...
5. **Semantic Differential** menuntut pasangan kata sifat yang benar-benar berlawanan dalam bahasa responden.

6. **Guttman** menuntut urutan kumulatif yang deterministik, sehingga jarang cocok untuk konstruk psikologis, tetapi ide dasarnya masih bisa dieksekusi dengan IRT.
...
* ***Forced-choice*** — ketika *social desirability* menjadi ancaman utama, dan responden dipaksa memilih di antara dua hal yang sama-sama menarik.
